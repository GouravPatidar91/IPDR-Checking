...
# Example: blacklisted IPs (can be loaded from config/db)
BLACKLISTED_IPS = {"10.0.0.66", "192.168.1.100"}



# Rule thresholds (lowered for demo/testing)
MAX_CONNECTIONS_PER_MIN = 5
MAX_CONNECTIONS_PER_10MIN = 10
MAX_TOTAL_CONNECTIONS = 50  # Flag if total connections between A↔B exceeds this
...
def detect_anomalies(df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    for (a, b), group in grouped:
        ...
        # Sliding window for 1-min interval
        for i in range(len(times)):
            ...
            if len(window) > MAX_CONNECTIONS_PER_MIN:
                ...
                if key not in seen:
                    ...
                    suspicious_ips.update([a, b])
                break
        # Sliding window for 10-min interval (repeated connections)
        for i in range(len(times)):
            ...
            if len(window) > MAX_CONNECTIONS_PER_10MIN:
                ...
                if key not in seen:
                    ...
                    suspicious_ips.update([a, b])
                break
        # Blacklisted IPs (flag even if only one connection)
    ...
    # If no anomalies found but there are suspicious IPs, add a generic anomaly for each
    ...
    # If still no anomalies, flag the most frequent IP as a fallback
